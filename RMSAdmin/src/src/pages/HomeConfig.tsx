...
import { BannerTable, BannerForm, CourseSelectTable, HotCourseTable, HotCourseForm, HomeReview, CourseSearch, Loading } from 'components';
import { IBannerCourseReqModel, IBannerCourseRspModel, ICourseQueryOption, ICourseRspModel, IHotCourseReqModel, IHotCourseRspModel } from 'models';
...
export const HomeConfig : FunctionComponent = () => {
  ...
  const handleChangeSequence = (banner: IBannerCourseRspModel, isUp: boolean) => {
    let relatedBanner: IBannerCourseRspModel;
    if(isUp) {
      relatedBanner = bannerModels?.filter(c=>c.sequence < banner.sequence).slice().pop()!;
    }
    else{
      relatedBanner = bannerModels?.find(c=>c.sequence > banner.sequence)!;
    }
  ...
  const handleChangeHotCourseSequence = (item: IHotCourseRspModel, isUp: boolean) => {
    ...
    item.sequence = related.sequence;
    ...
    promiseList.push(HomeService.updateHotCourse(related.id!, relatedInput));
                  ...
                  <Row>
                    <Col span={24}>
                      <HotCourseTable 
                        ...
                        changeSequence={handleChangeHotCourseSequence} />
                    </Col>
                  </Row>
                </TabPane>
              </Tabs>
