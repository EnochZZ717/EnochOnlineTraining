...
import { ConfirmModal, CourseFormFooter, TagSelect, CourseUser, ImageSelect, ImageSelectV2, KeywordInput, VideoSelect, VideoSelectV2 } from "components";
  ...
  return (
    <>    
      ...
      >
        ...
        >
          <Input style={{width: '99.5%'}} disabled={isPublished} placeholder="请输入名称" onChange ={(val: any) => editConfirm(update, { name: val.target.value})}/>
        </Form.Item>      
        <Form.Item 
          label="简介"
          name="description"
        ...
        >
          <Input.TextArea style={{width: '99.5%'}} disabled={isPublished} rows={4} value={currentCourse?.description} onChange={(val: any) => editConfirm(update, { description: val.target.value})}></Input.TextArea>
        </Form.Item>
        <Form.Item 
          label="封面" 
          name="coverImage"
