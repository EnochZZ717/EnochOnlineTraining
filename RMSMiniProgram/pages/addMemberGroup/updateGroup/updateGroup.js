...
Page({
  ...
  onLoad(option) {
    ...
    memberGroupService.getGroupDetail(option.id, this);
  },
  addCourse() {
    wx.navigateTo({
      url: `../../memberGroup/editGroupCourse/editGroupCourse?id=${this.data.id}&name=${this.data.name}`,
    })
  },
  onSave() {
    ...
    memberGroupService.updateGroup(data, this);
  },
  onReady: function () {

  },
  onShow: function () {
    var selctedCourse = wx.getStorageSync('updatedCourses');
    if (selctedCourse) {
      this.setData({
  ...
  removeCourse(e) {
    ...
    wx.setStorageSync('updatedCourses', courses);
    this.drag = this.selectComponent('#drag');
    this.drag.init();
  },
  toggleFixed(e) {
